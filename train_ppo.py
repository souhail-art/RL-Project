...
class PPOActorCritic(nn.Module):
    def __init__(self, input_shape, n_actions):
        ...
        # Dynamically compute conv output size
        with torch.no_grad():
            dummy_input = torch.zeros(1, *input_shape).to(torch.float32)
            conv_out = self.shared(dummy_input)
...
class PPOTrainer:
    ...
    def plot_metrics(self, rewards, losses):
        os.makedirs("results/ppov1", exist_ok=True)
        ...
        plt.tight_layout()
        plt.savefig("results/ppov1/metrics_plot.png")
        plt.close()
        ...
        df.to_csv("results/ppov1/metrics.csv", index=False)
        print("📊 Courbes et CSV enregistrés dans le dossier 'results/ppov1'")
    ...
    def save_model_weights(self):
        file_path = os.path.join(self.folder_path, "ppov1.pth")
...
if __name__ == "__main__":
    ...
    model = PPOActorCritic(obs_space, out_channels)
